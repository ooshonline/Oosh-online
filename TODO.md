...
- [ ] **M8 · Parent report share card.** A shareable image (generated client-side via Canvas API) showing the child's Level, streak, and top story. Parent taps "Share Progress" on the Profile screen. Uses `canvas.toDataURL()` → `navigator.share()`. No server needed; pure client-side. Good for social proof / organic acquisition.
