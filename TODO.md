...
- [ ] **F10 · "Read Later" count badge.** Small number badge on the Library bottom-nav tab showing how many stories are bookmarked (hidden when 0). Keeps the Read Later list top-of-mind without requiring navigation. Pairs with F7.
...
- [x] **U8 · Animated XP counter on Profile — SHIPPED (2026-09-18, commit `ef6641a`).** Count-up from 0 → state.xp over 600ms (ease-out cubic) on Profile navigation. Pulse animation (xpLand / scale 1.15) on landing. Respects prefers-reduced-motion. `#xp-counter-val` id on the XP stat card; `runProfileAnims()` function called from render(). LIVE.
...
**X1–X8 all shipped. Replenishment ideas (X9–X11):**
- [x] **X6 · "Read aloud" on the celebration screen.** ✅ Already shipped as part of X8 build — `renderTalkPrompt()` has had a `celeb-talk-say` speaker button calling `speakText(p.en, 0.85)` since the X8 session. Noted 2026-09-21.
- [x] **X7 · Smooth story-end transition. ✅ 2026-09-21 `5763906`.** `renderStoryComplete()` — full-bleed navy screen with 🌟 + "Story complete! / お話おわり！", 400ms scale-in animation, auto-advances to celebration after 900ms. `story-complete` added to FULLBLEED_SCREENS + navigate() preserve lists. 2 UI_STRINGS en+ja. LIVE (commit `5763906`).
...
- [ ] **X9 · Progress celebrations at reading milestones.** Show a small toast when the learner completes their 1st, 10th, 25th, 50th, 100th story — bilingual, mentions their streak or level, adds +XP. Pairs well with the existing XP milestone toasts.
- [ ] **X10 · "Hint" button on hard quiz questions.** A single-use hint per question (costs 0 XP, just reveals a relevant line from the story). Only available on quiz questions marked as inference — reduces friction for learners who get stuck without giving away the answer.
...
- [ ] **M9 · Streak milestone share prompt.** When a learner hits a 7-day or 30-day streak milestone, show an optional prompt: "Share your streak!" — reuses `generateShareCard()` from M8 but draws a streak-focused variant (flame emoji, streak count prominent). Pure client-side; no new infrastructure. Good viral acquisition hook.
